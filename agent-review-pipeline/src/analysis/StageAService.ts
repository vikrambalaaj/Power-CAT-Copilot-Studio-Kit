...
function detectYamlKind(component: { data?: string; content?: string }): string | undefined {
  const yaml = component.data || component.content || '';
  const match = /^\s*kind\s*:\s*(\w+)/m.exec(yaml);
  return match?.[1];
}

function detectToolType(parsed: Record<string, unknown>): ToolType {
  const actionKind = (parsed.action as Record<string, unknown> | undefined)?.kind as string | undefined;
  switch (actionKind) {
...
function detectKnowledgeSourceType(parsed: Record<string, unknown>): { sourceType: KnowledgeSourceType; url?: string } {
  const source = parsed.source as Record<string, unknown> | undefined;
  const kind = source?.kind as string | undefined;
  if (kind === 'SharePointSearchSource') return { sourceType: 'SharePoint', url: source?.site as string | undefined };
  if (kind === 'WebSearchSource' || kind === 'WebSource') return { sourceType: 'Web', url: source?.url as string | undefined };
  if (kind === 'DataverseSource') return { sourceType: 'Dataverse' };
  if (kind === 'FileSource' || kind === 'DocumentSource') return { sourceType: 'File' };
 ...
 */
export class StageAService implements IStageAService {
  /**
   * Execute Stage A from pre-parsed ZIP data.
   */
  async executeStageAFromData(
  ...
  generateLocalPatterns(botData: ParsedBotData): LocalPattern[] {
    ...
    const agentsWithoutDescription = (botData.childAgents ?? []).filter(
      (agent) => !agent.description || agent.description.trim() === '',
    );
  ...
  private async parseBotComponents(
    components: BotComponent[],
    botId: string,
    botName: string,
  ): Promise<ParsedBotData & { topicComponents: StageBTopicComponent[] }> {
    const topics: string[] = [];
    const topicComponents: StageBTopicComponent[] = [];
    const tools: string[] = [];
    const knowledgeSources: string[] = [];
    const testCases: string[] = [];
    const toolDetails: ToolDetail[] = [];
    const knowledgeDetails: KnowledgeDetail[] = [];
    const childAgents: ChildAgentDetail[] = [];
    const testCaseDetails: TestCaseDetail[] = [];
    ...
    for (const component of components) {
      ...
      if (componentType === ComponentType.CustomGPT) {
        ...
        if (yamlSource && !agentInstructions) {
          try {
            const parsed = yamlParsingService.parseYAML(yamlSource) as Record<string, unknown> | null;
            if (parsed) {
        ...
        if (yamlKind === 'AgentDialog') {
          try {
            const yamlSource = component.data || component.content || '';
            const parsed = yamlSource ? (yamlParsingService.parseYAML(yamlSource) as Record<string, unknown> | null) : null;
            const description = (parsed?.beginDialog as Record<string, unknown> | undefined)?.description as string | undefined;
        ...
        if (yamlKind === 'TaskDialog') {
          ...
          try {
            ...
            toolDetails.push({
              name,
              displayName: parsed?.modelDisplayName as string | undefined,
              description: parsed?.modelDescription as string | undefined,
        ...
        if (topicYaml) {
          try {
            const parsed = yamlParsingService.parseYAML(topicYaml) as Record<string, unknown> | null;
            if (parsed) {
              const topicComp: StageBTopicComponent = {
                TopicName: name,
                ModelName: (parsed.modelDisplayName as string | undefined) ?? undefined,
                ModelDescription: (parsed.modelDescription as string | undefined) ?? undefined,
              };
              ...
              const inputVars = parsed.inputType
                ? (parsed.inputType as Record<string, unknown>).properties
              ...
              const outputVars = parsed.outputType
                ? (parsed.outputType as Record<string, unknown>).properties
              ...
              if (Array.isArray(parsed.triggerQueries)) {
                topicComp.triggerQueries = parsed.triggerQueries as string[];
              }
        ...
        componentType === ComponentType.BotEntity ||
        ...
        try {
          ...
          if (parsed?.kind === 'EvaluationData') {
            ...
            const input = (firstRow?.input as string | undefined) ?? name;
            const source = (firstRow?.source as string | undefined) === 'Generated' ? 'Generated' : 'Manual';
