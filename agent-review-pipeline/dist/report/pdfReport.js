...
function hr(doc, y, margin, pageWidth) {
...
function sectionHeading(doc, text, y, margin) {
    doc.setFontSize(13);
 ...
 */
function renderAgentReport(doc, agentName, scores, stageB, stageC, isFirst) {
...
export function generatePdfReport(results) {
    ...
    results.forEach((result, i) => {
        renderAgentReport(doc, result.stageA.botName ?? 'Unknown Agent', result.scores, result.stageB, result.stageC, i === 0);
    });
    // Footer on all pages
    const pageWidth = doc.internal.pageSize.getWidth();
    const pageHeight = doc.internal.pageSize.getHeight();
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(TEXT_MID);
        doc.text(`Agent Review Pipeline — Page ${i} of ${totalPages}`, pageWidth / 2, pageHeight - 8, { align: 'center' });
    }
