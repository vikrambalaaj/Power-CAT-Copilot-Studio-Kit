...
export function calculatePatternScore(evaluation) {
    if (!Array.isArray(evaluation?.Patterns) || !evaluation.Patterns.length)
        return 0;
...
export function calculateInstructionScore(evaluation) {
    ...
    for (const i of evaluation.issues) {
        if (!i || !i.id || typeof i.id !== 'string' || !i.id.trim())
            return 0;
...
export function calculateScores(stageBResult, stageCResult, threshold = DEFAULT_THRESHOLD, options) {
    ...
    const validIssues = Array.isArray(stageCResult?.issues) && stageCResult.issues.every(i => i && typeof i.id === 'string' && i.id !== 'missing-instruction-input' &&
        COMPLIANCE_CRITERIA.some(c => i.id === c.id || i.id.startsWith(c.id)));
