...
## 8. Corrections and additional required cases (revision 2)

The current source already implements identity argument binding, mailbox-scoped clients, atomic subscription claims, reminder dispatch, production signing-key guards and a rationale toggle. Their tests are regression/acceptance tests, not proof that those features are absent. The audit report revision 2 supersedes earlier implementation claims.
...
| EVA-05 / P0 | Supply a fabricated reference, real inaccessible reference and valid reference with mismatched numeric value. | None becomes verified merely because a reference is nonempty; only independently resolved, authorized and matching evidence receives verified status. |
| TRC-06 / P1 | Render the actual decision card for low, unknown, conflicting and high confidence, with zero/mixed verified sources. | Text/color reflect validated evidence; no fixed “100% match” or green status misrepresents uncertainty. Toggle displays the correct decision's concise rationale and sources. |
